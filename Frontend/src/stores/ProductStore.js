...
export const useBurgerStore = defineStore('burger', {
  ...
  actions: {
    ...
    async fetchBurgerById(id) {
      try {
        const response = await axios.get(`http://localhost:7168/api/ProductAPI/${id}`);
    ...
    async fetchCategories() {
      try {
        const response = await axios.get(' http://localhost:7168/api/CategoryAPI');
    ...
    async update() {
      try {
        ...
        const response = await axios.put(
          `http://localhost:7168/api/ProductAPI/${this.currentBurger.Id}`,
          this.currentBurger,
          {
            headers: {
